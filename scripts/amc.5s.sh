...
# Shows "Deck 3●" in the menu bar (live count, ! prefix when something waits).
DATA=$(curl -s -m 2 http://localhost:7777/summary)
if [ -z "$DATA" ]; then echo "Deck ⏻"; echo "---"; echo "Dashboard offline"; exit 0; fi
python3 - "$DATA" << 'PY'
import json, sys
d = json.loads(sys.argv[1])
flag = "!" if d["waiting"] else ""
print(f"Deck {flag}{d['live']}\u25CF")
print("---")
for w in d["waiting"]:
